...
class NotifyHandler(metaclass=SingletonMeta):
    ...
    async def _delete_threads_past_deadline(self):
        ...
        for member_id, thread_dict in threads.items():
            for thread_id, deadline in thread_dict.items():
                deadline = datetime.datetime.strptime(deadline, "%Y-%m-%d").replace(
                    tzinfo=timezone
                )
    ...
    async def convert_tagged_threads(
        ...
        dict[str, list[discord.Member] | datetime.datetime],
    ]:
        """
        ...
        for member_id, thread_dict in threads.items():
            for thread_id, deadline in thread_dict.items():
                ...
                deadline = (
                    datetime.datetime.strptime(deadline, "%Y-%m-%d").replace(
                        tzinfo=timezone
                    )
                    if deadline
                    else None
                ...
                converted[thread]["members"].append(
                    self.guild.get_member(int(member_id))
                )
        return converted
    ...
    async def notify_now_to_channel(
        self,
        ...
        contents_0 = [
            await self._get_notify_content(0, data, thread)
            for thread, data in refined[0].items()
        ]
        contents_1 = [
            await self._get_notify_content(1, data, thread)
            for thread, data in refined[1].items()
        ]
        contents_3 = [
            await self._get_notify_content(3, data, thread)
            for thread, data in refined[3].items()
        ]
        contents_5 = [
            await self._get_notify_content(5, data, thread)
            for thread, data in refined[5].items()
        ]
        ...
        if contents and interaction:
            ...
            formatted_message = (
                str(set([message for message in messages if message]))
                .replace("{", "")
                .replace("}", "")
                .replace("'", "")
            )
            await interaction.response.send_message(
                embeds=embeds, content=formatted_message
            )
            ...
            await interaction.response.send_message(
                ...
                delete_after=5.0,
                embed=discord.Embed(
                    title="通知するものがありませんでした。", color=discord.Color.blue()
                ),
            )
            ...
            await interaction.response.send_message(
                ...
                delete_after=5.0,
                embed=discord.Embed(
                    title="通知するものがありませんでした。",
                    description="想定外のケースです。よろしければ管理者に連絡してください。",
                    color=discord.Color.yellow(),
                ),
            )

    # 呼び出す側がループを回す
    async def _notify_for_one_channel(
        self,
        days: int,
        data: dict[
            discord.Thread | discord.TextChannel,
