...
class EmbedHandler(metaclass=SingletonMeta):
    ...
    def get_embed_ping(self):
        data = {
            ...
            "Guild": f"{self.interaction.guild.name} (`{self.interaction.guild.id}`)"
            if self.interaction.guild
            else "DM",
            "Channel": f"{self.interaction.channel.mention} (`{self.interaction.channel.id}`)"
            if self.interaction.guild
            else "DM",
        }
        ...
        if step == 1:
            ...
            if not step:
                embed = self.get_embed_error(
                    title="エラーが発生しました (step is None or invalid)"
                )
            elif not self.interaction:
                embed = self.get_embed_error(
                    title="エラーが発生しました (interaction is None or invalid)"
                )
            else:
                embed = self.get_embed_error(
                    title="エラーが発生しました (unknown error)"
                )
        return embed
    ...
    def get_embed_get_tagged_threads(
        self, step: int, threads: dict[str, str] | None = None
    ):
        if step == 1:
            embed = discord.Embed(
                title="1. メンバーを選択",
                ...
                for thread, deadline in threads.items():
                    if self.interaction.guild.get_channel_or_thread(int(thread)):
                        target_channels.append(
                            self.interaction.guild.get_channel_or_thread(int(thread))
                        )
                        channel_deadlines.append(deadline)

                timezone = datetime.timezone(datetime.timedelta(hours=9))
                self.channels = [
                    f'・{channel.mention} \n    提出期限 : {deadline}\n    残り : {str((datetime.datetime.strptime(deadline, "%Y-%m-%d").replace(tzinfo=timezone) - datetime.datetime.now(timezone)).days + 1)} 日'
                    for channel, deadline in zip(target_channels, channel_deadlines)
                ]
                embed = discord.Embed(
                    title="**取得結果：**\n"
            ...
            if not step:
                ...
                embed = self.get_embed_error(
                    title="エラーが発生しました (threads is None or invalid)"
                )
            elif not self.interaction:
                embed = self.get_embed_error(
                    title="エラーが発生しました (interaction is None or invalid)"
    ...
    def get_embed_get_tagged_members(
        self, step: int, members: dict[str, Any] | None = None
    ):
        if step == 1:
            embed = discord.Embed(
                title="1. 投稿を選択",
            ...
            if not members:
                ...
                members = [
                    f"・ {self.interaction.guild.get_member(int(member_id)).mention}"
                    for member_id in target_member_ids
                ]
                num_of_days = (
                    str(
                        (
                            datetime.datetime.strptime(raw_deadline, "%Y-%m-%d")
            ...
            if not step:
                ...
                embed = self.get_embed_error(
                    title="エラーが発生しました (members is None or invalid)"
                )
            elif not self.interaction:
                embed = self.get_embed_error(
                    title="エラーが発生しました (interaction is None or invalid)"
    ...
    def format_member_threads(self, refined_threads: dict[str, dict[str, str]]):
        ...
        def format_member(member_id, threads: dict[str, str]):
            ...
            if member:
                threads_str = "\n - ".join(
                    thread
                    for thread in (
                        format_thread(thread_id, deadline)
                        for thread_id, deadline in threads.items()
                    )
                    if thread
                )
                if threads_str:
        ...
        return "\n\n".join(
            member
            for member in (
                format_member(member_id, threads)
                for member_id, threads in refined_threads.items()
            )
            if member
        )
    ...
    def get_embed_all_tagged_members(self, step: int | None = None):
        if step == 1:
            ...
            refined_threads = {
                member_id: threads
                for member_id, threads in all_threads.items()
                if threads
            }
    ...
    def get_embed_invite(self, step: int | None):
        if step == 1:
            ...
            embed = discord.Embed(
                title="リンクを表示しました", color=discord.Color.green()
            )
        else:
            if not step:
                embed = self.get_embed_error(
    ...
    def get_embed_error(self, title: str = None, description: str = None):
        return discord.Embed(
            ...
            description="もう一度やり直してください。何度もエラーが出る場合は管理者に連絡してください"
            if not description
            else description,
    ...
    def get_embed_success(
        self, task: str = None, title: str = None, description: str = None
    ):
