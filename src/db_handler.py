...
class DBHandler:
    def __init__(
        self, table_name: str | None = None, region_name: str = "ap-northeast-1"
    ):
    ...
    ) -> dict[str, str | int | float | list[str] | list[int] | list[float]] | None:
        try:
    ...
    ) -> list[dict[str, str | int | float | list[str] | list[int] | list[float]]]:
        try:
